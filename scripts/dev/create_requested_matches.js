...
const matchesToCreate = [
  ...
  {
    title: 'F7 La Satalia contra Replayers',
    venue: 'Satalia',
    date: 'Dom, 10 may',
    time: '10:00',
    max_players: 14,
...
async function main() {
  ...
  console.log('Inserting', dataToInsert.length, 'matches...');
  const { data, error } = await supabase.from('matches').insert(dataToInsert).select();
  ...
  console.log('Successfully inserted matches:', data.map(m => `${m.date} ${m.time} - ${m.title}`).join('\n'));
}
